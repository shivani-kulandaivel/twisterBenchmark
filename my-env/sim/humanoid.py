...
class HumanoidSim:
    ...
    def step_physics(self, substeps: int = PHYSICS_SUBSTEPS) -> None:
        for _ in range(substeps):
            ...
            self._apply_targets()
            if self._assist_strength > 0.0:
                self._apply_balance_assist()
            mujoco.mj_step(self.model, self.data)

    def set_balance_assist_strength(self, strength: float) -> None:
    ...
    def _apply_balance_assist(self) -> None:
        ...
        s = self._assist_strength
        self.data.xfrc_applied[pelvis, 0] = float(np.clip(s * fx, -BALANCE_F_MAX, BALANCE_F_MAX))
        self.data.xfrc_applied[pelvis, 1] = float(np.clip(s * fy, -BALANCE_F_MAX, BALANCE_F_MAX))
        self.data.xfrc_applied[pelvis, 3] = float(np.clip(s * tx, -BALANCE_T_MAX, BALANCE_T_MAX))
        self.data.xfrc_applied[pelvis, 4] = float(np.clip(s * ty, -BALANCE_T_MAX, BALANCE_T_MAX))

    def com_xyz(self) -> tuple[float, float, float]:
        ...
        cz = float(sum(self.model.body_mass[i] * self.data.subtree_com[i][2] for i in range(self.model.nbody)) / total)
        return cx, cy, cz
    ...
    def get_contacts(self) -> list[dict[str, Any]]:
        ...
        for c in self.data.contact[: self.data.ncon]:
            ...
            contacts.append(
                {
                    "geom1": mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_GEOM, g1) or str(g1),
                    "geom2": mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_GEOM, g2) or str(g2),
